%configurationfile
\pagestyle{AuHandin}

\title{Dokumentation}
%author will mostly not be used unless we are talking solo work and then group members should be disabled ;)
\author{Forfatter Navn}
%Vejlederens navn
\SetVejleder{Søren Hansen}
%Skole/Universitetets navn
\SetUniversity{Aarhus Universitet}
%Projektnavn
\SetProject{Semesterprojekt 3. Semester}
%Gruppe Nummer
\SetGroupNumber{10}
%Add the names of all the members in the group, can handle 1 to 8 members, names are added one per set of []
\GroupMemberNames[\fontsize{17}{17}\selectfont Jacob Munkholm Hansen][\fontsize{15}{15}\selectfont Halfdan Vanderbruggen Bjerre][\fontsize{9}{9}\selectfont Mikkel Espersen][\fontsize{9}{9}\selectfont Ahmad Sabah]{}
%add student numbers of the students, must have the exact same order as the names listed above.
\GroupMemberStudentNo[\fontsize{17}{17}\selectfont 201404796][\fontsize{15}{15}\selectfont 20091153][\fontsize{9}{9}\selectfont 201507348][\fontsize{9}{9}\selectfont 201209619]{}