...
Testen for forbindelsen mellem PSoC master og PSoC slave gennemføres vha. en terminal og logic analizer på Analog Discovery. De to PSoc enheder bliver sat 
til hver ders PC, og der laves en UART forbindelse så det kan ses på et terminal-vindue hvilke data der bliver sendt og modtaget. Derefter forbindes de to PSoC
enheder til hinanden med SPI, og der tilføjet desuden en fælles GND forbindelser. På billedet ses en også en VDD forbindelse fra PSoC master, denne er dog kun 
nødvendig hvis slaven ikke får VDD fra PC. 
...
går lav ved dataoverførelse, og den korrekte bitsekvens sendes på MOSI. Grunden til der ses efter om SS går lav, er fordi der ved SPI kan være flere slaver 
tilsluttet. SPI fungere på den måde at den trække sinalet lavt på den slave som skal modtage signalet.
...
Der sendes data fra master til slave, og der kigges på terminalen og logic analizer for at se hvilke data slave modtager. I denne test sendes kommandoen 0x5 til 
slaven at antal gange. 

\subsection{Test af SPI forbindelse imellem Devit8000 og PSoC1}

Testen for SPI forbindelsen imellem DK8k og MASTER foretages med Analog Aiscovery, hvor der måles på SS, CLCK, og henholdsvis MISO og MOSI.
...
Ved testen ses at der udskrives 05 på terminalvindue for PSoC-slave. Dette passer fint med den kommando der sendes fra master. SPI forbindelsen fungere altså
fint.
...
Der kigges også på logic analizer, og det ses at SS går lav ved dataoverførelse. Og der sendes desuden værdien 0x5 på MOSI. Igen virker det som forventet.   
